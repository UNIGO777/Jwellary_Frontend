...
export default function AdminSilverRates() {
  ...
  const [purityMark, setPurityMark] = useState(999)
  const [purityPercent, setPurityPercent] = useState('99.9')
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/admin/silver-rates?${params.toString()}`, withAdminAuth())
  ...
  const startEdit = (row) => {
    ...
    setPurityMark(Number(row?.purityMark || 999))
    setPurityPercent(row?.purityPercent !== undefined ? String(row.purityPercent) : '99.9')
    setRatePerKg(row?.ratePerKg !== undefined ? String(row.ratePerKg) : '')
  }

  const canSave = Boolean(ratePerKg) && !saving
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (editingId) await api.put(`/api/admin/silver-rates/${editingId}`, withAdminAuth({ body: payload }))
      else await api.post('/api/admin/silver-rates', withAdminAuth({ body: payload }))
      await load()
      resetForm()
    } catch (err) {
      setError(getErrorMessage(err))
  ...
  const remove = async (id) => {
    if (!window.confirm('Delete this silver rate?')) return
    try {
      await api.del(`/api/admin/silver-rates/${id}`, withAdminAuth())
    ...
    <div>
      ...
      <form onSubmit={submit} className="mb-8 space-y-5 rounded-lg border border-stone-200 bg-white p-6 shadow-sm">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            <label className="block text-sm font-medium text-stone-700">Purity Mark *</label>
            <select
              value={purityMark}
              onChange={(e) => setPurityMark(Number(e.target.value))}
              className="mt-1 h-10 w-full rounded-md border border-stone-300 bg-white px-3 text-sm text-stone-900 outline-none"
            >
              {[999, 925, 900, 800].map((m) => (
          ...
          <div>
            <label className="block text-sm font-medium text-stone-700">Rate / Kg (INR) *</label>
            <input
              value={ratePerKg}
          ...
          <table className="min-w-full divide-y divide-stone-200">
            <thead className="bg-stone-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-stone-500">Mark</th>
                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-stone-500">Purity</th>
                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-stone-500">Rate / Kg</th>
                ...
                rows.map((r) => {
                  ...
                  return (
                    <tr key={String(id)} className="hover:bg-stone-50">
                      ...
                      <td className="px-6 py-4 text-sm font-medium text-stone-900">{r?.purityMark ?? '-'}</td>
                      <td className="px-6 py-4 text-sm text-stone-700">{r?.purityPercent ?? '-'}</td>
                      <td className="px-6 py-4 text-sm text-stone-700">{r?.ratePerKg ?? '-'}</td>
